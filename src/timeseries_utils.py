...
def generate_metric_for_channels(
    ...
    column_name = "delta_views" if metric == "views" else "delta_subs" if metric == "subs" else "delta_videos"
...
def plot_metric(
    ...
    for game_name in game_names:
        if channel:
            metrics = generate_metric_for_channels(games_timeseries_df=df, game_name=game_name, period=period, window=window, metric=metric)
            x = metrics["period"]
    ...
    if channel:
        title = f"{metric.capitalize()} Generated by {', '.join(game_names)} channels"
    else:
        title = f"{', '.join(game_names)} Videos {metric.capitalize()}"
...
def game_percentage(df: pl.DataFrame, channels: bool = False):
    ...
    with_game_counts = df.get_column(column_name).value_counts().sort("count", descending=True).head(10)
...
def metric_to_json(
    ...
    """
    if channel_views:
        views = generate_metric_for_channels(games_timeseries_df=df, game_name=game_name, period=period, window=window, metric=metric)
    ...
    # Construct json
    chart_json = json_format(game_name, x_axis_data, y_axis_data, mark_area_data, metric)
...
def json_format(
    ...
    chart_json = {
        "title": {
            "text": f"{metric.capitalize()} Generated by {game_name.capitalize()}",
        },
...
def export_views_top_games_json(df: pl.DataFrame, game_names: str, period: str = "W", window: int = 3,cutoff="2017-01-01", output_file="weekly_delta_views.json"):
    """
    ...
    Returns:
        None
    """
    result = {"weeks": None}  # Dictionary to store the data
    # Loop through each game and collect delta_views
    for game_name in game_names:
        metric = generate_metric_for_channels(games_timeseries_df=df, game_name=game_name, period=period, window=window, metric="views")
        result[game_name] = metric["delta_views"].astype(int).tolist()
        result["weeks"] = metric["period"].astype(str).tolist()
    # Export to JSON
    with open(output_file, "w") as f:
