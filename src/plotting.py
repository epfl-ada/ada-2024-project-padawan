...
def plot_views(
    ...
    """
    Plots the weekly views for a specified game as well as events that occurred during the period.
    ...
    fig.add_trace(
        go.Scatter(x=x, y=y, mode="lines", name=label, line=dict(color="blue"))
    )
    ...
    if dates:
        for event_name, date in dates:
            ...
            # Add event name as text annotation on top of the line
            fig.add_trace(
                go.Scatter(
                    x=[event_date],
                    y=[y.max()],
    ...
    title = f"Views Generated by {game_name} channels" if channel else f"{game_name} Videos Views"
    # Customize the layout
    fig.update_layout(
        title=title,
        xaxis_title="Month",
        yaxis_title="Views",
        xaxis=dict(tickangle=45, showgrid=False),  # Enable gridlines for x-axis
